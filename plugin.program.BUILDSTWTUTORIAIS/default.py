...
def writeAdvanced(name, url):
	...
	if ADVANCEDWORKING == True:
		...
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Gostaria de Fazer o Download e Instalar [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Install[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
		if choice == 1:
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		...
		if yes:
			...
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Para Salvar e Ativar feche o KODI, Pressione OK para Fechar o Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if warning == True:
			...
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Gostaria de Fazer o Download e Instalar :' % COLOR2 + '\n[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Sim, Instalar[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			#buildzips = []
			...
			if int(float(percent)) > 0:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Para Salvar e Ativar feche o KODI, Pressione OK para Fechar o Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
			else:
				if isinstance(errors, str):
					error = error.encode('utf-8')
...
def freshStart(install=None, over=False):
	...
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Click [B][COLOR springgreen] - Sim - [/COLOR][/B]" % COLOR2 + "\nPara apagar sua Build atual, \r\nEm Seguida sera Instalada [COLOR %s]%s[/COLOR]!!" % (COLOR1, install), nolabel='[B][COLOR red]Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Sim[/COLOR][/B]')
...
class Guiwiz(pyxbmct.AddonDialogWindow):
	...
	def main_buttons(self):
		self.BuildsButton= pyxbmct.Button('[COLOR %s][B]Builds TWT[/B][/COLOR]' % MAIN_BUTTONS_TEXT, focusTexture=FBUTTON,noFocusTexture=BUTTON)
		self.placeControl(self.BuildsButton,-2 , 1 , 13, 8)
		self.connect(self.BuildsButton, lambda: self.BuildList())

		self.MaintButton = pyxbmct.Button('[COLOR %s][B]Limpeza[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		...
		self.ToolsButton = pyxbmct.Button('[COLOR %s][B]Ferramentas[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		...
		self.CloseButton = pyxbmct.Button('[COLOR %s][B]Fechar[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
	...
	def BuildList(self):
		...
		if not BUILDFILE == 'http://' and not BUILDFILE == '':
			...
			ThemeButton = pyxbmct.Button('[COLOR %s][B]Updates[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
